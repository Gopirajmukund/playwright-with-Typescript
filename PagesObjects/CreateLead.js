...
exports.leadIdDetails = class LeadPage {
    ...
    constructor(page) {
        ...
        this.requestButton = this.page.locator("//button[text()='Requests']")
        this.EmpIdFilterInput = this.page.locator("[aria-label='EMP ID Filter Input']")
        this.approveButton = this.page.locator("//button[text()='Approve']")
        this.scrollBar = this.page.locator('[class="ag-horizontal-right-spacer ag-scroller-corner"]')
    ...
    async clickOnRequestNutton(){
        await this.requestButton.click()
    }
    async FilterEmpId(EmpId){
        await this.EmpIdFilterInput.click()
        await this.EmpIdFilterInput.fill(EmpId)
    }
    async scroll(){
        await this.page.setViewportSize({ width: 1910, height: 1080 })
    }

    async ApproveEmpLeave()
    {
        await this.page.waitForSelector("//button[text()='Approve']")
